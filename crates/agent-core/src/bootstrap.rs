...
pub fn resolve_project_config(root: &Path) -> Result<ProjectConfig, BootstrapError> {
    ...
    if config.system_prompt.is_none() {
        if let Some(prompt) = load_root_agents_md(root)? {
...
"#,
        ...
        assert_eq!(config.runtime.request.max_output_tokens, Some(2048));
        assert_eq!(config.runtime.request.temperature, Some(0.4));
        assert_eq!(
            config
                .runtime
                .request
