...
use crate::capabilities::{initialize_response, CONFIG_LOOP, CONFIG_MODEL, CONFIG_THOUGHT_LEVEL};
...
impl<C: AgentCore> AgentCoreAcpBackend<C> {
    ...
    async fn emit(&self, session_id: &acp::SessionId, update: acp::SessionUpdate) -> Result<(), acp::Error> {
        let (ack_tx, ack_rx) = oneshot::channel();
        self.session_update_tx
            .send((acp::SessionNotification::new(session_id.clone(), update), ack_tx))
    ...
    fn prompt_text(blocks: &[acp::ContentBlock]) -> String {
        let text = blocks
            ...
            .trim()
            .to_owned();
        if text.is_empty() { "(empty prompt)".to_owned() } else { text }
    }

    fn session_title_from_prompt(prompt_text: &str) -> String {
        let trimmed = prompt_text.trim();
        if trimmed.is_empty() { return "ACP Session".to_owned(); }
        if trimmed.len() > 48 { format!("{}...", &trimmed[..48]) } else { trimmed.to_owned() }
    }

    async fn resolve_project(&self, cwd: &std::path::Path) -> Result<agent_store::Project, acp::Error> {
        self.core.resolve_or_create_project(cwd.to_path_buf()).await.map_err(map_store_error)
    }

    async fn config_options_for_session(&self, session_id: agent_store::SessionId) -> Result<Vec<acp::SessionConfigOption>, acp::Error> {
        let current_model = self.core.current_model_id_for_session(session_id).await.map_err(map_store_error)?;
        let current_loop_name = self.core.current_loop_name_for_session(session_id).await.map_err(map_store_error)?;
        let runtime_config = self.core.effective_runtime_config(session_id).await.map_err(map_store_error)?;
...
impl<C: AgentCore> acp::Agent for AgentCoreAcpBackend<C> {
    ...
    async fn authenticate(&self, _arguments: acp::AuthenticateRequest) -> Result<acp::AuthenticateResponse, acp::Error> {
    ...
    async fn new_session(&self, arguments: acp::NewSessionRequest) -> Result<acp::NewSessionResponse, acp::Error> {
        ...
        let session = self.core.create_session(project.id).await.map_err(map_store_error)?;
        let session_id = session_id_from_ulid(session.id);

        self.emit(&session_id, acp::SessionUpdate::SessionInfoUpdate(
            acp::SessionInfoUpdate::new().updated_at(session.updated_at.to_rfc3339())
        )).await?;

        Ok(acp::NewSessionResponse::new(session_id).config_options(self.config_options_for_session(session.id).await?))
    }
    ...
    async fn load_session(&self, arguments: acp::LoadSessionRequest) -> Result<acp::LoadSessionResponse, acp::Error> {
        ...
        let messages = self.core.messages(session_id).await.map_err(map_store_error)?;
        ...
        Ok(acp::LoadSessionResponse::new().config_options(self.config_options_for_session(session.id).await?))
    }

    async fn list_sessions(&self, arguments: acp::ListSessionsRequest) -> Result<acp::ListSessionsResponse, acp::Error> {
    ...
    async fn prompt(&self, arguments: acp::PromptRequest) -> Result<acp::PromptResponse, acp::Error> {
        ...
        let mut stream = self.core.turn(session_id, messages).await.map_err(map_store_error)?;
        ...
        while let Some(event) = stream.next().await {
            match mapper.map(event) {
                ...
                MappedEvent::Cancelled => return Ok(acp::PromptResponse::new(acp::StopReason::Cancelled)),
                MappedEvent::Failed(message) => return Err(internal_error(message)),
                MappedEvent::TurnComplete(stop_reason) => return Ok(acp::PromptResponse::new(stop_reason)),
            }
        }
    ...
    async fn cancel(&self, arguments: acp::CancelNotification) -> Result<(), acp::Error> {
        let session_id = parse_session_id(&arguments.session_id)?;
        self.core.cancel_turn(session_id).await.map_err(map_store_error)
    }

    async fn set_session_config_option(&self, arguments: acp::SetSessionConfigOptionRequest) -> Result<acp::SetSessionConfigOptionResponse, acp::Error> {
        let session_id = parse_session_id(&arguments.session_id)?;
        ...
        Ok(acp::SetSessionConfigOptionResponse::new(self.config_options_for_session(session_id).await?))
    }
}
