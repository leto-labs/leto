...
use provider_openai::{AssistantCreateRequest, AssistantTool, Client, Config};
...
#[tokio::test]
async fn create_assistant_posts_request_and_parses_response() {
    let response_body = serde_json::json!({
        ...
        "model": "gpt-4o",
        "instructions": "Use the knowledge base to answer support questions.",
        "tools": [
            { "type": "file_search" }
        ],
        "metadata": {
    ...
    let response = client
        .assistants()
        .create(&AssistantCreateRequest {
            name: Some("Support Helper".into()),
            description: Some("Answers support questions".into()),
            instructions: Some("Use the knowledge base to answer support questions.".into()),
            tools: vec![AssistantTool::file_search()],
            metadata: BTreeMap::from([("team".into(), "support".into())]),
            ..AssistantCreateRequest::default()
        })
        .await
        .unwrap();
    ...
    assert!(request_head.contains("post /v1/assistants http/1.1"));
    ...
    assert_eq!(response.name.as_deref(), Some("Support Helper"));
    assert_eq!(response.tools, vec![AssistantTool::file_search()]);
}
