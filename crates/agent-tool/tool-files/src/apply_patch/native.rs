...
fn parse_v4a_patch(patch: &str) -> Result<Vec<PatchOp>, ToolError> {
    ...
    while index < end {
        ...
        if let Some(path) = line.strip_prefix(UPDATE_FILE) {
            ...
            if index < end && lines[index].starts_with(MOVE_TO) {
                move_to = Some(resolve_patch_path(
                    lines[index]
            ...
            while index < end && !is_operation_header(lines[index]) {
                    ...
                    && !lines[index].starts_with("@@")
                {
                    ...
                    let (prefix, body) = current
                        .chars()
                        .next()
                        .map(|prefix| (prefix, &current[prefix.len_utf8()..]))
                        .ok_or_else(|| tool_failed("invalid empty patch line"))?;
                    match prefix {
                        ' ' => {
                            old_lines.push(body.to_owned());
                            new_lines.push(body.to_owned());
                        }
                        '-' => old_lines.push(body.to_owned()),
                        '+' => new_lines.push(body.to_owned()),
                        _ => {
                            return Err(tool_failed(format!(
                                "invalid patch line prefix '{prefix}' in {}",
                                path.display()
                            )));
                        }
                    }
        ...
        return Err(tool_failed(format!("unrecognized patch operation: {line}")));
    }
...
fn resolve_patch_path(raw: &str) -> Result<PathBuf, ToolError> {
...
fn tool_failed(message: impl Into<String>) -> ToolError {
