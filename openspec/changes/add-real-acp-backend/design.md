...
## Decision: First Real Slice Uses Brain Directly

The real ACP backend should depend directly on `brain-core::Brain`, not on
`brain-server`.

That keeps the ACP path close to the core engine and avoids pulling HTTP/server
concerns into the ACP integration surface.
