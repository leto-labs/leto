...
implemented in the workspace, but the TUI frontend described here is not yet.
...
no subcommand SHALL launch the TUI in local in-process mode using
`BrainServer::client()`.

The TUI is a `BrainApi` client. It does NOT implement the `Transport` trait.

#### Scenario: Default local TUI
- **WHEN** `brain` is run with the `tui` feature enabled and no subcommand
- **THEN** it SHALL start the TUI against an in-process `BrainApi` client

#### Scenario: Engine boundary remains `BrainApi`
- **WHEN** the TUI is running
- **THEN** it SHALL communicate with the engine exclusively through `BrainApi`

### Requirement: CLI Subcommands
The binary SHALL support the existing provider, credential, and session
commands, and it SHALL also support:

- `serve` — start the HTTP/SSE server
- `attach <url>` — launch the TUI against a remote `BrainApi` endpoint

#### Scenario: Serve mode
- **WHEN** `brain serve` is run
- **THEN** it SHALL start the existing HTTP/SSE server

#### Scenario: Attach mode
- **WHEN** `brain attach http://127.0.0.1:PORT` is run
- **THEN** it SHALL launch the TUI against that remote endpoint
...
### Requirement: TuiApp
The system SHALL provide a `TuiApp` that acts as a thin client of `BrainApi`
and renders an interactive full-screen terminal UI using ratatui +
crossterm.
...
- deterministic integration tests for local and attached modes
