...
type = "openai-oauth"
# No API key needed — uses stored OAuth tokens
# Optional overrides:
# callback_port = 1455
# account_id = "..."
```
...
- **Requires**: `add-config-system` (OAuth provider declared in config, token store path from config)
- **Required by**: `add-brain-cli` (login/logout subcommands, provider auto-discovery from stored tokens)
