...
## Deferred (blocked on add-config-system)

- Config file integration (`type = "openai-oauth"` in provider config)
- Auto-discovery of stored OAuth tokens during `Brain::discover()`
- Token store path from config (hardcode `.agents/auth/` for now)

These are thin wiring layers. The core OAuth implementation is self-contained
and fully functional without them — callers just construct the provider
manually instead of going through config.
