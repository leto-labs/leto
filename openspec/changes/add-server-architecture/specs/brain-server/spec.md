...
The `brain-cli` dual-mode behavior SHALL be tracked in `add-brain-cli` and `add-tui-transport` change specs.
In this worktree, `brain-server` SHALL define and expose only server API behavior.

#### Scenario: Server-only runtime
- **WHEN** `brain-server` behavior is validated today
- **THEN** it SHALL expose server-only HTTP/API functionality
- **AND** SHALL not require a TUI transport to run the API
