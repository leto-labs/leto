...
The system SHALL include an `EchoTool` in the `brain-tools` crate (moved from `brain-loops`) that logs its invocation via `tracing` and returns the input message as a string. This tool implements `Tool` directly with no driver trait. `brain-loops` SHALL re-export `EchoTool` for backward compatibility.
