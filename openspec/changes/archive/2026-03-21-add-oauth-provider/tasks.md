...
- Browser and device OAuth flows were validated manually against a real `nori-cli` + `brain-acp` OAuth connection rather than tracked as required mock-server integration tests in this change.
- Runtime discovery from stored credentials is already wired through the CLI and ACP provider discovery paths; richer provider-declaration syntax, if added later, belongs to `add-config-system`.
