...
- Harbor should be treated as the practical operational harness for the first
  benchmark workflow: it now uses six reference surfaces in this repo
  (`codex`, `mini-swe-agent`, `terminus-2`, `agent`, `codex-acp`,
  `agent-acp`) through the dedicated runner script `scripts/harbor-run.sh` and the thin
  ...
  `agent` is the direct repo-local execution surface, `codex-acp` is the main
  external ACP comparison path, `mini-swe-agent` and `terminus-2` are useful
  built-in comparison surfaces, and `agent-acp` is the repo-local ACP
  loop-validation path.
