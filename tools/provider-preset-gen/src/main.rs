...
struct ProviderSpec {
    ...
    supported_api_surfaces: &'static [&'static str],
}
...
const PROVIDERS: &[ProviderSpec] = &[
    ProviderSpec {
        ...
        supported_api_surfaces: &["responses", "chat_completions"],
    },
    ProviderSpec {
        module: "groq",
        const_name: "GROQ",
    ...
    ProviderSpec {
        ...
        supported_api_surfaces: &["chat_completions"],
    },
    ProviderSpec {
        module: "together",
        const_name: "TOGETHER",
...
fn render_provider_module(provider: &ProviderSpec, models: &[GeneratedModel]) -> String {
    ...
    out.push_str("use provider::{ModelCost, ModelInfo, ModelLimit};\n\n");
    out.push_str("use super::{OpenAiConfigPreset, REASONING_LMH};\n");
    out.push_str("use crate::config::OpenAiApiSurface;\n\n");
    ...
    out.push_str(&format!(
        ...
        render_api_surfaces(provider.supported_api_surfaces)
    ));
    out.push_str("};\n");
    out
}

fn render_api_surfaces(values: &[&str]) -> String {
    let items = values
        .iter()
        .map(|value| match *value {
            "responses" => "OpenAiApiSurface::Responses".to_owned(),
            "chat_completions" => "OpenAiApiSurface::ChatCompletions".to_owned(),
            other => panic!("unsupported API surface {other}"),
        })
        .collect::<Vec<_>>()
        .join(", ");
    format!("&[{items}]")
...
fn render_model(model: &GeneratedModel) -> String {
    ...
    out.push_str(&format!(
        "        family: {},\n",
        render_opt_cow_str(m.family.as_deref())
    ));
    out.push_str(&format!(
        "        reasoning_efforts: {},\n",
        if m.reasoning {
            "Cow::Borrowed(REASONING_LMH)"
        } else {
            "Cow::Borrowed(&[])"
        }
    ));
    ...
    out.push_str(&format!(
        "        knowledge: {},\n",
        render_opt_cow_str(m.knowledge.as_deref())
    ));
    out.push_str(&format!(
        "        release_date: {},\n",
        render_opt_cow_str(m.release_date.as_deref())
    ));
    out.push_str(&format!(
        "        last_updated: {},\n",
        render_opt_cow_str(m.last_updated.as_deref())
    ));
    out.push_str(&format!(
        "        open_weights: Some({}),\n",
    ...
    out.push_str(&format!(
        "        status: {},\n",
        render_opt_cow_str(m.status.as_deref())
    ));
...
fn render_opt_cow_str(value: Option<&str>) -> String {
    match value {
        Some(value) => format!("Some(Cow::Borrowed({value:?}))"),
        None => "None".to_owned(),
    }
}
...
fn render_str_slice(values: &[String]) -> String {
    let items = values
        ...
        .collect::<Vec<_>>()
        .join(", ");
    format!("Cow::Borrowed(&[{items}])")
}

fn render_cost(cost: Option<&CostToml>) -> String {
    match cost {
        Some(cost) => format!(
            "Some(ModelCost {{ input: {}, output: {}, reasoning: {}, cache_read: {}, cache_write: {}, input_audio: {}, output_audio: {} }})",
...
fn render_limit(limit: Option<&LimitToml>) -> String {
    match limit {
        Some(limit) => format!(
            "Some(ModelLimit {{ context: {}, input: {}, output: {} }})",
...
fn render_mod_rs() -> String {
    ...
    out.push_str("use crate::config::{Config, OpenAiApiMode, OpenAiApiSurface};\n\n");
    ...
    out.push_str("    pub supported_api_surfaces: &'static [OpenAiApiSurface],\n");
    out.push_str("}\n\n");
    ...
    out.push_str("    }\n\n");
    out.push_str("    pub fn into_config(self, api_key: impl Into<String>) -> Config {\n");
    out.push_str("        Config {\n");
    ...
    out.push_str("            supported_api_surfaces: self.supported_api_surfaces,\n");
    out.push_str("            api_surface_mode: OpenAiApiMode::Auto,\n");
    out.push_str("        }\n");
    out.push_str("    }\n");
    out.push_str("}\n");
    out
